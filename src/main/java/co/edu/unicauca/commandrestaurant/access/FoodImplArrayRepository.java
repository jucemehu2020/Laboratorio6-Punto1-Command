...
public class FoodImplArrayRepository implements IFoodRepository {
    ...
    /**
     * Elimina al cliente con todos sus atributos
     *
     * @param component del cliente
     * @return true si lo elimino correctamente, false de lo contrario
     */
    @Override
    public boolean deleteFood(Food component) {
        int i = 0;
        for (Food componentAux : components) {
            if (componentAux.getId() == component.getId()) {
                components.remove(i);
